...
var coords = window.message;
var s = new String(coords);

var x = parseFloat(s.split(",")[0]);
var z = parseFloat(s.split(",")[1]);

map.setView([x,z], 20);

addresses.forEach(address => {
  if(address.location == coords){
    const marker = L.marker(address.location).addTo(map);
    marker.bindPopup(`<b>Adresse: ${address.address}</b><br><b>GBL: ${address.gbl}</b><br><b>Kürzel: ${address.con}</b><br><b>Kunden: ${address.customers}</b>`).openPopup();
  } else {
    const marker = L.marker(address.location).addTo(map);
    marker.bindPopup(`<b>Adresse: ${address.address}</b><br><b>GBL: ${address.gbl}</b><br><b>Kürzel: ${address.con}</b><br><b>Kunden: ${address.customers}</b>`);
  }
});
